...
| charon.enr.existingSecret | object | `{"name":"","privateKeyDataKey":"private-key","publicKeyDataKey":"public-enr"}` | Point to an existing Kubernetes secret that holds the ENR private key. If 'privateKey' above is not set and this 'existingSecret.name' is provided, 'generate' is ignored. NOTE: If not set, the chart will automatically check for a secret named 'charon-enr-private-key' (configurable via secrets.defaultEnrPrivateKey) and use it if it exists. |
| charon.enr.existingSecret.privateKeyDataKey | string | `"private-key"` | Key in the secret's 'data' field holding the private key hex string |
| charon.enr.existingSecret.publicKeyDataKey | string | `"public-enr"` | Key in the secret's 'data' field holding the public ENR string |
| charon.enr.generate | object | `{"annotations":{},"enabled":true,"image":{"pullPolicy":"IfNotPresent","repository":"obolnetwork/charon","tag":"v1.6.0"},"kubectlImage":{"pullPolicy":"IfNotPresent","repository":"bitnami/kubectl","tag":"1.33.3"},"nodeSelector":{}}` | Enable automatic generation of an ENR private key. Only used if 'privateKey' and 'existingSecret.name' are not provided. The generated key will be stored in a secret with data keys 'private-key' and 'public-enr'. |
